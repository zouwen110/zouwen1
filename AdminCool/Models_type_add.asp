...
sql="select * from web_models_type where ( FolderName='"&FolderName&"' and FolderName<>'' )or FileName='"&l_url&"'"
rs.open(sql),cn,1,3
if not rs.eof then
response.Write "<script language='javascript'>alert('模板文件名重复或目标文件夹重复，请重新命名！');history.go(-1);</script>"
else
rs.addnew
rs("name")=l_name
rs("FileName")=l_url
